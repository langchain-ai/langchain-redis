...
def test_redis_vector_store_with_config(
    ...
    config = RedisConfig(
        index_name=f"test_index_{uuid.uuid4().hex}", redis_url=redis_url
    )
...
def test_redis_vector_store_from_documents_with_config(
    ...
    config = RedisConfig(metadata_schema=metadata_schema, redis_url=redis_url)
...
def test_redis_config_with_custom_schema(
    ...
    config = RedisConfig.from_schema(schema, redis_url=redis_url)
    vector_store = RedisVectorStore(embeddings, config=config)
...
def test_redis_config_with_yaml_schema(
    ...
    config = RedisConfig.from_yaml(str(schema_path), redis_url=redis_url)
    vector_store = RedisVectorStore(embeddings, config=config)
