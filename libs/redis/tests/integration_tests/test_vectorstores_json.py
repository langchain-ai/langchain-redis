...
def test_redis_from_existing(texts: List[str], redis_url: str) -> None:
    ...
    vector_store.index.schema.to_yaml("test_schema_json.yml")
...
def test_similarity_search_with_metadata_filtering(redis_url: str) -> None:
    ...
    metadata1 = {
        key: int(value) if key == "msrp" else value
        for key, value in output1[0].metadata.items()
        if key != "text"
    }

    assert metadata1 == metadatas[1]
    ...
    metadata2 = {
        key: int(value) if key == "msrp" else value
        for key, value in output2[0].metadata.items()
        if key != "text"
    }
    assert metadata2 == metadatas[0]
    ...
    metadata3 = {
        key: int(value) if key == "msrp" else value
        for key, value in output3[0].metadata.items()
        if key != "text"
    }
    assert metadata3 == metadatas[2]
...
def test_connect_to_redisvl_created_index_w_index_name(redis_url: str) -> None:
    ...
    for (label, text), vector in zip(sentences.items(), vectors):
        data.append({"text": text, "embedding": vector})
